...
│   ├── lambda/
│   │   └── client.go            # Post(): Bearer-authenticated HTTP POST, 3-min timeout
│   ├── config/
...
- **Boundary re-auth:** always re-authenticates before `authorize-session` to pick up freshly granted Entra group memberships; retries on 403 for up to 3 minutes to handle PIM propagation delays.
